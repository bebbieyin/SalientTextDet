...
from modeling.model import build_text_model
from modeling.data.dataloader import build_dataloader
from modeling.loss import bce_ohem
...
def train_epoch(train_dataloader, model, optimizer, scheduler, device, thresh, epoch):
    ...
    for idx, (_,images,text_gt) in enumerate(tqdm(train_dataloader)):
...
def test(valid_dataloader,model, thresh):
    ...
    model.eval()
    for _, (_,images,text_gt) in enumerate(tqdm(valid_dataloader)):
...
if __name__ == '__main__':
    ...
    preprocess =transforms.Compose([
        transforms.ToTensor(),
        transforms.Resize((config['INPUT']['SIZE'],config['INPUT']['SIZE'])),
    ])
    ...
    # load model
    model = build_text_model(backbone_cfg=config['MODEL']['BACKBONE_CFG'], input_size=config['INPUT']['SIZE'], device=device).to(device)
    ...
    filename = os.path.join(output_path,'logs.csv')
